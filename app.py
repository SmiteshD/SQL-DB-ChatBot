...
radio_opt=["Use SQLLite 3 Database-Student.db","Connect your SQL Database"]
...
if radio_opt.index(selected_opt)==1:
    ...
    mysql_user=st.sidebar.text_input("Provide MY SQL Username")
...
if not db_uri:
    st.info("Plese select the database and enter the information correctly")

if not api_key:
    st.info("Please enter the Groq API Key correctly")
...
def configure_db(db_uri,mysql_host=None,mysql_user=None,mysql_password=None,mysql_db=None):
    if db_uri==LOCALDB:
        ...
        if not(mysql_host and mysql_user and mysql_password and mysql_db):
            st.error("Please provide all the MySQL details correctly.")
            st.stop()
